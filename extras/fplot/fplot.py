#!/usr/bin/env python3
"""fplot - Terminal function plotter with selectable renderers.

Supports chaining multiple functions via JSON piping:
    fplot "sin(x)" --json | fplot "cos(x)" --color red --json | fplot "x/3"
"""
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog='Chain multiple functions: fplot "sin(x)" --json | fplot "cos(x)" --color red\n'
               'Parametric curves: fplot -p "cos(t),sin(t)" -l\n'
               'For expressions starting with -, use: fplot -- "-2*x"',
    )
